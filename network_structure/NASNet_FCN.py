...
class NormalCell1(nn.Module):
    ...
    def forward(self, x):
        ...
        x1 = self.block1_left(h) + self.block1_right(h)       #17
        x2 = self.block2_left(prev) + self.block2_right(h)    #16
        x3 = self.block3_left(h) + self.block3_right(h)       #17
        x4 = self.block4_left(prev) + self.block4_right(prev) #17
        x5 = self.block5_left(prev) + self.block5_right(prev) #16

        return torch.cat([prev, x1, x2, x3, x4, x5], 1), x   #17,17,16,17,17,16
...
class NASNet_FCN(nn.Module):
    ...
    def _make_normal(self, block, block1, repeat, output, removeFC=False):
        """make normal cell
        Args:
            block: cell type
        ...
        if removeFC:
            layers.append(block1(self.x_filters, self.prev_filters, output_channels=17))
        return layers
    ...
    def _make_layers(self, repeat_cell_num, reduction_num):
        ...
        for i in range(reduction_num):
            layers.extend(self._make_normal(NormalCell, NormalCell1, repeat_cell_num, self.filters))
            self.filters *= 2
            layers.append(self._make_reduction(ReductionCell, self.filters))

        layers.extend(self._make_normal(NormalCell, NormalCell1, repeat_cell_num, self.filters, removeFC=True))
